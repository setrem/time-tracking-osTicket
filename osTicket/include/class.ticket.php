...
implements RestrictedAccess, Threadable, Searchable {
    ...
    // CHANGED!
    function getTicketTimeTrackingRunning($task_id) {
        global $thisstaff;

        $where_task = '';
        if ($task_id) {
            $where_task = ' AND task_id='.db_input($task_id);
        } else {
            $where_task = ' AND task_id IS NULL';
        }

        $ticket_id = $this->getId();
        $staff_id = $thisstaff->getId();
        $sql = 'SELECT start_time FROM '.TICKET_TIME_TRACKING_TABLE
    ...
    function isTicketTimeTrackingRunning($task_id) {
        $ticket_running = $this->getTicketTimeTrackingRunning($task_id);
        $count_tickets_opened = db_num_rows($ticket_running);
        return $count_tickets_opened > 0;
    }

    function stopTicketTimeTrackingRunning($task_id) {
        global $thisstaff;
        ...
        // $start_time_ticket_running = db_result($this->getTicketTimeTrackingRunning($task_id));
        $sql = 'UPDATE '.TICKET_TIME_TRACKING_TABLE.' SET '
            .' end_time = NOW() WHERE '
            .' staff_id = '.db_input($staff_id)
            .' AND end_time IS NULL';
            // .' AND ticket_id = '.db_input($ticket_id)
            // .$where_task
            // .' AND start_time = '.db_input($start_time_ticket_running);
        db_query($sql);
    }

    function startStopTicketTimeTracking($start, $task_id) {
        global $thisstaff;

        $ticket_id = $this->getId();
        $staff_id = $thisstaff->getId();

        $this->stopTicketTimeTrackingRunning($task_id);
        if ($start) {
            $sql = 'INSERT INTO '.TICKET_TIME_TRACKING_TABLE
                .' (ticket_id,'
                .($task_id ? ' task_id,' : '')
                .' staff_id, start_time) VALUES'
                .' ('.db_input($ticket_id)
                .($task_id ? ', '.db_input($task_id) : '')
                .', '.db_input($staff_id)
                .', NOW())';
            db_query($sql);
        }
