...
foreach ($groups as $g=>$desc) { ?>
    <li class=""><a href="#<?php echo Format::slugify($g); ?>"
...
<?php
echo '
<div class="tab_content" id="time-tracking">
  <table class="dashboard-stats table" id="table-time-tracking">
    <tbody>
      <tr>
        <th width="30%" class="flush-left">
            Pessoa
        </th>
        <th>Ticket</th>
        <th>Tarefa</th>
...
foreach ($groups as $g=>$desc) {
    ...
    <div class="tab_content hidden" id="<?php echo Format::slugify($g); ?>">
...
<script>
    ...
    // CHANGED!
    function timeTrackingStaffs(cb) {
      $.get("ajax.php/staff/time-tracking-staffs")
          .done(function (data) {
            var table = $('#table-time-tracking tbody:last');
            table.empty();
            var staffs = JSON.parse(data);
            if (staffs.length === 0) {
              var tr = '\
                <tr>\
                  <th colspan="4" style="text-align: center">Não há time tracking rodando.</th>\
                </tr>\
              ';
              table.append(tr);
            } else {
              for (var i = 0; i < staffs.length; i++) {
                ...
                var tr = '\
                  <tr>\
                    <th class="flush-left">#NAME</th>\
                    <td>#TICKET</td>\
                    <td>#TASK</td>\
                    <td>#START</td>\
                  </tr>\
                ';
                tr = tr.replace('#NAME', staff.staff);
                tr = tr.replace('#TICKET', '<a href="tickets.php?id=' + staff.ticket_id + '" target="_blank" rel="noopener noreferrer">' + staff.ticket_title + '</a>');
                if (!!staff.task_id) {
                  tr = tr.replace('#TASK', '<a href="tasks.php?id=' + staff.task_id + '" target="_blank" rel="noopener noreferrer">' + staff.task_title + '</a>');
                } else {
                  tr = tr.replace('#TASK', '');
                }
                tr = tr.replace('#START', staff.start_time);
                table.append(tr);
              }
            }
            if (cb) {
