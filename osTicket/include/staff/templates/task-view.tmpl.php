...
$id = $task->getId();
// CHANGED!
$show_time_tracking_control = !!$task->ticket;
$ticket_task_time_tracking_running = $show_time_tracking_control ? $task->isTaskTimeTrackingRunning() : false;
// CHANGED!
$dept = $task->getDept();
       ...
       <div class="content">
        ...
        <div class="flush-right">
            ...
            <form
                ...
                onclick="this.submit()"
                title="<?php echo __($ticket_task_time_tracking_running ? 'Stop' : 'Start'); ?>"
                action="tasks.php?id=<?php echo $task->getId(); ?>">
                <?php csrf_token(); ?>
                <input type="hidden" name="a" value="start-stop-on-ticket" />
                <a href="#" onclick="this.parentNode.submit()">
                    <i class="<?php echo __($ticket_task_time_tracking_running ? 'icon-stop' : 'icon-play'); ?>"></i>
                </a>
            </form>
            <!-- CHANGED! -->
            <a  id="task-view"
            ...
            <?php
                ...
                <!-- CHANGED! -->
                <?php if ($show_time_tracking_control) { ?>
                    <form
                        method="POST"
                        enctype="multipart/form-data"
                        class="action-button"
                        ...
                        action="tasks.php?id=<?php echo $task->getId(); ?>">
                        <?php csrf_token(); ?>
                            <input type="hidden" name="a" value="start-stop" />
                            <a href="#" onclick="this.parentNode.submit()">
                                <i class="<?php echo __($ticket_task_time_tracking_running ? 'icon-stop' : 'icon-play'); ?>"></i>
                            </a>
                    </form>
