...
implements AuthenticatedUser, EmailContact, TemplateVariable, Searchable {
    ...
    // CHANGED!
    function isTimeTrackingRunning() {
        $sql = 'SELECT * FROM '.TICKET_TIME_TRACKING_TABLE
                .' WHERE staff_id='.db_input($this->getId())
                .' AND end_time IS NULL';
        return db_fetch_row(db_query($sql));
    }

    function shouldStartOrStopTimeTracking() {
        $sql = 'SELECT * FROM '.STAFF_SHIFT_TABLE
                .' WHERE staff_id='.db_input($this->getId())
                .' AND (
                    (
                        TIMESTAMPDIFF(MINUTE, shift1_start, TIME(NOW())) BETWEEN 0 AND 10
                        OR TIMESTAMPDIFF(MINUTE, shift2_start, TIME(NOW())) BETWEEN 0 AND 10
                    )
                    AND NOT EXISTS(
                        SELECT * FROM '.TICKET_TIME_TRACKING_TABLE
                        .' WHERE '.TICKET_TIME_TRACKING_TABLE.'.staff_id='.db_input($this->getId())
                        .' AND '.TICKET_TIME_TRACKING_TABLE.'.end_time IS NULL
                    )
                OR (
                (
                    TIMESTAMPDIFF(MINUTE, shift1_end, TIME(NOW())) BETWEEN 0 AND 10
                    OR TIMESTAMPDIFF(MINUTE, shift2_end, TIME(NOW())) BETWEEN 0 AND 10
                )
                AND EXISTS(SELECT * FROM '.TICKET_TIME_TRACKING_TABLE
                    .' WHERE '.TICKET_TIME_TRACKING_TABLE.'.staff_id='.db_input($this->getId())
                    .' AND '.TICKET_TIME_TRACKING_TABLE.'.end_time IS NULL
                ))
            )';
