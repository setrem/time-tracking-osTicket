...
if($_POST && !$errors):
    if ($task) {
        ...
        case 'start-stop':
            if ($task->ticket) {
                $ticket_time_tracking_running = $task->ticket->isTicketTimeTrackingRunning($task->getId());
                $task->ticket->startStopTicketTimeTracking(!$ticket_time_tracking_running, $task->getId());
                
                $msg = __($ticket_time_tracking_running ? 'Time tracking stoped successfully.' : 'Time tracking started successfully.');
                $redirect = 'tasks.php?id='.$task->getId();
            }
            break;
        case 'start-stop-on-ticket':
            if ($task->ticket) {
                $ticket_time_tracking_running = $task->ticket->isTicketTimeTrackingRunning($task->getId());
                $task->ticket->startStopTicketTimeTracking(!$ticket_time_tracking_running, $task->getId());
